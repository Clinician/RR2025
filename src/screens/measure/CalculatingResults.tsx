...
interface PPGProcessingResult {
  ...
  statistics?: import('../../types/PPGData').PPGStatistics;
}

const CalculatingResultsScreen: React.FC<CalculatingResultsScreenProps> = ({ onComplete, onCalibrationComplete, videoPath }) => {
  ...
  useEffect(() => {
    ...
    const processVideoFrames = async (videoPath: string, metadata: any): Promise<PPGProcessingResult> => {
      ...
      // Calculate statistics using the PPGDataExportService
      const tempResult = {
        rawSignals: allSignals,
        frameCount: allSignals.length,
        processingTime,
        qualityWarnings
      };
      
      const ppgData = ppgDataExportService.formatPPGData(tempResult);
  ...
  const handleSharePPGData = async (filename: string) => {
    ...
    if (isCalibrationMode && referenceMeasurement) {
      // Calculate calibration offset instead of sharing data
      setTimeout(() => {
    ...
    setTimeout(async () => {
      try {
        // Apply calibration offsets to measurement results if not in calibration mode
        let adjustedResult = ppgProcessingResult;
        if (!isCalibrationMode) {
          adjustedResult = await applyCalibrationOffsets(ppgProcessingResult);
        }
  ...
  // Apply calibration offsets to measurement results
  const applyCalibrationOffsets = async (result: PPGProcessingResult): Promise<PPGProcessingResult> => {
    try {
      const storedOffsets = await CalibrationOffsetService.loadOffsets();
      
      if (!storedOffsets) {
        console.log('No stored calibration offsets found, using raw results');
        return result;
      }

      // Calculate raw estimated values
      const rawSystolic = calculateEstimatedSystolic(result);
      const rawDiastolic = calculateEstimatedDiastolic(result);

      // Apply offsets
      const adjustedValues = CalibrationOffsetService.applyOffsets(
        rawSystolic,
        rawDiastolic,
        storedOffsets
      );

      console.log('Applied calibration offsets:', {
        raw: { systolic: rawSystolic, diastolic: rawDiastolic },
        adjusted: adjustedValues,
        offsets: { 
          systolic: storedOffsets.systolicOffset, 
          diastolic: storedOffsets.diastolicOffset 
        }
      });

      // Create adjusted result with calibrated values
      const adjustedResult = {
        ...result,
        calibratedMeasurement: adjustedValues,
        appliedOffsets: {
          systolicOffset: storedOffsets.systolicOffset,
          diastolicOffset: storedOffsets.diastolicOffset,
          timestamp: storedOffsets.timestamp
        }
      };

      return adjustedResult;
    } catch (error) {
      console.error('Error applying calibration offsets:', error);
      return result;
    }
  };
  ...
  const handleCalibrationCompletion = () => {
    if (!ppgProcessingResult || !referenceMeasurement) {
      console.error('Missing data for calibration completion');
      onComplete();
      return;
    }

    try {
      // Calculate estimated blood pressure from PPG signals
      const estimatedSystolic = calculateEstimatedSystolic(ppgProcessingResult);
      const estimatedDiastolic = calculateEstimatedDiastolic(ppgProcessingResult);

      // Calculate offset between reference measurement and phone measurement
      const systolicOffset = referenceMeasurement.systolic - estimatedSystolic;
      const diastolicOffset = referenceMeasurement.diastolic - estimatedDiastolic;
      ...
      console.log('Calibration completed:', {
        ...
        estimated: { systolic: estimatedSystolic, diastolic: estimatedDiastolic },
  ...
  const calculateEstimatedSystolic = (result: PPGProcessingResult): number => {
    // Use the CESM library when it's available
    return 120;
  };

  const calculateEstimatedDiastolic = (result: PPGProcessingResult): number => {
    // Use the CESM library when it's available
    return 80;
  };
